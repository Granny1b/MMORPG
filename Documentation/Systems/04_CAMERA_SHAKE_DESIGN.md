...
## Part 2 — The cursor-aim problem

This is the part that decides whether camera shake is viable in this game at all.
...
**The fix:** the shaker caches the pre-shake pose each frame before applying the offset, and exposes
it. `TopDownAimController` is ours, so `TryGetCursorWorldPosition` can build its ray from the
unshaken pose and ignore the shake entirely — the screen wobbles, the crosshair does not lie.

The kit's click-to-pick raycast keeps the jitter, because fixing it would mean editing
`PlayerCharacterController_Inputs.cs`. That is acceptable: it samples only on the frame of a click,
so the worst case is a click landing a few pixels off during a stomp. Record it as a known limit
rather than patching `Core/`.
...
CameraShaker                    (MonoBehaviour on TopDownGameplayCamera.prefab root — ours)
  ...
  └─ exposes UnshakenPosition / UnshakenRotation for aiming
...
TopDownAimController            (ours) builds its cursor ray from the unshaken pose
```
...
2. **Fix the aim.** Cache the unshaken pose, point `TopDownAimController.TryGetCursorWorldPosition`
   at it, then shake hard and check the character's facing holds still. **This is the step that
   decides whether shake is usable in this game**, so it comes before anything networked.
3. **`PlayAt` with distance falloff**, plus the summed clamp and the instance cap.
4. **Tier 1 end to end.** `CameraShakeEmitter` on a monster skill's effect prefab, wired through
...
5. **The player setting**, wired to a slider in `UIDialogs_G.prefab`, with 0 fully disabling.
6. **Tier 2**, only once step 4's limits actually bite — a scripted phase cue with no effect prefab
   to hang on.
7. **Tier 3**, only for a real zone-wide event.

Steps 1–4 are a day's work and cover the boss stomp. Steps 6 and 7 may never be needed.

## Open decisions

- **Is camera shake acceptable at all in a cursor-aim game?** Step 2 answers it empirically. If
  decoupling the aim still leaves it feeling bad, the fallback is rotation-only shake at low
  amplitude, or a post-process wobble that never touches the transform.
